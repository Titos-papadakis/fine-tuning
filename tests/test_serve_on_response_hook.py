...
def test_on_response_fires_for_a_non_streaming_request(mock_client):
    ...
    assert len(calls) == 1
    req, text, p_tok, c_tok, elapsed_ms, customer_id, cache_hit = calls[0]
    assert req.messages[0].content == "hi"
    assert isinstance(text, str) and text
    assert p_tok > 0 and c_tok > 0
    assert elapsed_ms >= 0
    assert customer_id is None  # no auth configured in this fixture
    assert cache_hit is False  # caching is disabled in this fixture
...
def test_on_response_fires_for_a_streaming_request_with_the_full_text(mock_client):
    ...
    assert len(calls) == 1
    _req, text, p_tok, c_tok, elapsed_ms, customer_id, cache_hit = calls[0]
    ...
    assert elapsed_ms >= 0
    assert customer_id is None  # no auth configured in this fixture
    assert cache_hit is False  # streaming never uses the cache
