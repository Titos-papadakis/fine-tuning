...
`/health` carries what you need to tell two deployments apart: which profile answered, under
which regulatory regime, which backend, recent p50/p99, and the served/constrained request
counts. An operator staring at a wrong answer should not have to guess which model produced it.

### Running it without a GPU

`ftspec serve` assumes vLLM and CUDA. `scripts/serve_adapter.py` is the same endpoint with
three interchangeable backends, so you can verify an integration on a laptop:

```bash
# No weights at all -- schema-valid records from the profile's own generator.
python scripts/serve_adapter.py --profile saas_support --backend mock

# A freshly trained adapter, plain HuggingFace + grammar-constrained decoding.
...
It does not reimplement `/v1/chat/completions` — the FastAPI app is imported from
`ftspec.serving.serve`, so the mock exercises the same wire format, the same server-side schema
enforcement and the same system-prompt injection that production runs. A mock testing a
parallel implementation would prove nothing about the real one.

The mock backend labels itself in `/health`, in the model name (`...-MOCK`) and in the logs.
Its records are schema-valid and unrelated to the input: it tests plumbing, never accuracy.

### One-command smoke test

```bash
python scripts/test_client.py --profile saas_support --n 5
```

Uses the official `openai` SDK and sends **one user message containing a raw transcript** — no
system prompt, no schema, no `guided_json`. A valid record coming back is itself the proof that
the guarantee is server-side, because the client had no way to ask for it.

It discards a warm-up request (the SDK's first call is cold and would otherwise dominate the
median), then prints client-side latency next to the server's own p50/p99 and the difference
between them — transport and serialisation get quietly folded into "inference latency"
otherwise. Exits non-zero on any validation failure, so it works as a deploy gate.
...
scripts/
  serve_adapter.py             launcher: vllm | transformers | mock backends
  test_client.py               one-command smoke test via the official openai SDK
...
| | Status |
|---|---|
| Core engine, contracts, metric derivation, registry, redaction | ✅ 158 tests passing, CPU-only |
...
| Live endpoint driven by the official `openai` SDK, all three profiles | ✅ CI starts the server and validates every reply |
